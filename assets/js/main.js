...
function displayInfo(id) {
  //const id = element.id;
  console.log('dd', id)
  const gov = gouvernorats.filter((gov) => gov.idhtml === id).map((gov) => gov);

  document.getElementById('gov').textContent = gov[0]?.name;
  document.getElementById('description').textContent = gov[0]?.description;
  document.getElementById('places').textContent = gov[0]?.popularPlaces.join(", ");
  document.getElementById('plats').textContent = gov[0]?.popularDishes.join(", ");
  document.getElementById('traditions').textContent = gov[0]?.traditions.join(", ");
  console.log("ID de l'élément cliqué :", gov[0]);
